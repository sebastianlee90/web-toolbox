...
import { toolsList, type toolsType } from "@/constants/toolsList";
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="text-foreground text-lg font-bold">{item.name}</span>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {item.children?.map((subItem: toolsType, subIndex: number) => (
          <ToolItem key={subItem.id} subItem={subItem} subIndex={subIndex} />
        ))}
      </div>
    </motion.div>
  );
...
function ToolItem({
  ...
  const isSubItemInView = useInView(subRef, { once: true });

  return (
    <motion.div
      ref={subRef}
      initial={{ opacity: 0, y: 10 }}
      animate={isSubItemInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 10 }}
    ...
    >
      ...
      >
        ...
        <div className="flex flex-col gap-3">
          ...
          <h2 className="text-foreground/60 text-md">{subItem.desciption}</h2>
        </div>
      </Link>
    </motion.div>
  );
  ...
  return (
    <>
      {toolsList.map((item, index) => (
        <ToolCategory key={item.id} item={item} index={index} />
      ))}
    </>
  );
}
